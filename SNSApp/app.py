...
def signup_process():
    ...
    session['user_id'] = str(uuid.UUID(bytes=user_id))
...
def threads_view(user_id=None):                                 #user_idのデフォルト値をNoneに指定
    ...
    if current_user_id is None:
        ...
        if user_id is None:
            ...
            threads = Thread.get_all(user_id.bytes)
...
def create_thread(filepath=None):           #filepathのデフォルト値をNoneに設定
    ...
    if user_id is None:
        ...
        if title == "":
            ...
            Thread.create(thread_id, uuid.UUID(user_id).bytes, title, filepath, theme_id)
...
def thread_detail_view(thread_id):
    ...
    if current_user_id is None:
        ...
        #リアクション数を表示
        reaction_counts = Reaction.count(thread_id.bytes)
        #コメント数を表示
        comment_counts = Comment.count(thread_id.bytes)
        
        #ポストを表示
        posts = Post.get_all(thread_id.bytes)
        for post in posts:
            post["created_at"] = post["created_at"].strftime("%Y/%m/%d %H:%M")
            post["id"] = str(uuid.UUID(bytes=post["id"]))
            post["user_id"] = str(uuid.UUID(bytes=post["user_id"]))
        
        #コメントを表示
        comments = Comment.get_all(thread_id.bytes)
        for comment in comments:
            comment["created_at"] = comment["created_at"].strftime("%Y/%m/%d %H:%M")
            comment["user_name"] = User.get_name_by_id(comment["user_id"])
            comment["user_id"] = str(uuid.UUID(bytes=comment["user_id"]))
            comment["id"] = str(uuid.UUID(bytes=comment["id"]))
...
def delete_thread(thread_id):
    ...
    if user_id is None:
        ...
        if thread is None:
            abort(404)
        #自分のスレッドのみ削除可能
        elif str(uuid.UUID(bytes=thread["user_id"])) != user_id:
...
def create_comment(thread_id):
    ...
    if user_id is None:
        ...
        if content == "":
            ...
            Comment.create(comment_id, uuid.UUID(user_id).bytes, thread_id.bytes, content)
...
def delete_comment(thread_id, comment_id):
    ...
    if user_id is None:
        ...
        if comment is None:
            abort(404)
        elif str(uuid.UUID(bytes=comment["user_id"])) != user_id:
...
def create_post(thread_id):
    ...
    if content == "":
        ...
        Post.create(post_id, uuid.UUID(user_id).bytes, thread_id.bytes, content, image, count, rep)
...
def post_delete(post_id, thread_id):
    ...
    if str(uuid.UUID(bytes=post['user_id'])) != user_id:
...
def create_reaction(thread_id):
    ...
    #すでに送信済みのリアクションかを確認
    existing_reaction = Reaction.find_same_reaction(user_id_bytes, thread_id, reaction_id)

    if existing_reaction is None:
        #初めてリアクションを送る場合
        Reaction.create(user_id_bytes, thread_id, reaction_id)
        flash('リアクションを送信しました', 'success')
        return redirect(url_for("thread_detail_view", thread_id = thread_id))

    #すでに同一リアクションを送っている場合はDBをUPDATEする
    else:
        Reaction.update(user_id_bytes, thread_id, reaction_id)
        flash("リアクションを送信しました", "success")
        return redirect(url_for("thread_detail_view", thread_id = thread_id))
