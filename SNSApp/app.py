...
def threads_view(user_id=None):                                 #user_idのデフォルト値をNoneに指定
    ...
    if current_user_id is None:
        ...
        for thread in threads:
            ...
            thread_id_bytes = thread["id"].bytes
...
def create_thread(filepath=None):           #filepathのデフォルト値をNoneに設定
    ...
    if user_id is None:
        ...
        if title == "":
            ...
            return redirect(url_for("threads_view", user_id=uuid.UUID(bytes=user_id)))
...
def thread_detail_view(thread_id):
    ...
    if current_user_id is None:
        ...
        current_user_id = uuid.UUID(bytes=current_user_id)
        thread_id_bytes = thread_id.bytes
        #スレッドを表示
        thread = Thread.find_by_id(thread_id_bytes)
        if thread is None:
            abort(404)
        thread["id"] = thread_id #uuidのidを代入
        thread["created_at"] = thread["created_at"].strftime("%Y/%m/%d %H:%M")
        thread["user_name"] = User.get_name_by_id(thread["user_id"])
...
def complete_thread(thread_id):
    ...
    if user_id is None:
        ...
        thread = Thread.find_by_id(thread_id.bytes)
        if thread is None:
            abort(404)
            ...
            Thread.complete(thread_id.bytes)
...
def delete_comment(thread_id, comment_id):
    ...
    if user_id is None:
        ...
        comment = Comment.find_by_id(comment_id.bytes)
        if comment is None:
            abort(404)
            ...
            Comment.delete(comment_id.bytes)
...
def new_post_view(thread_id):
    ...
    if user_id is None:
        ...
        if thread is None:
            ...
            return render_template("/thread/post/new.html", thread_id=thread_id)
...
def create_post(thread_id, filepath=None, rep=None):
    ...
    if user_id is None:
        ...
        if content == "":
            ...
            Post.create(post_id, user_id, thread_id.bytes, content, filepath, count, rep)
...
def create_tweet():
    ...
    if current_user_id is None:
        ...
        if content == "":
            ...
            tweets_id = uuid.uuid4().bytes
            Tweet.create(tweets_id, current_user_id, content)
